...
if string.find(GetRealmName(),"Netherwing") then
	AimedDelay = 0.10;
end
...
Frame:SetScript("OnEvent",function()
	...
	if ((event == "UNIT_SPELLCAST_SUCCEEDED") or (event == "UNIT_SPELLCAST_START") or (event == "UNIT_SPELLCAST_STOP")) and arg1 == "player" then
		...
		elseif (arg2 == "Aimed Shot" and event == "UNIT_SPELLCAST_SUCCEEDED") then
			castdelay = autoshot_latency/1e3
			autoshot_latency_update();
			Swing_Start(AimedDelay);
		
		elseif _G[AddOn.."_Frame_Timer"]:GetAlpha() == 0 and (arg2 == "Steady Shot" or arg2 == "Multi-Shot" or arg2 == "Aimed Shot") and (event == "UNIT_SPELLCAST_STOP") then
			Cast_Interrupted();	
		end
	...
	if event == "UNIT_SPELLCAST_SENT" and arg1 == "player" then 
		...
		elseif arg2 == "Auto Shot" and castStart == false and swingStart == false and not UnitCastingInfo("player") and not IsCurrentSpell("Steady Shot") then
			Cast_Start()
		end
	end
